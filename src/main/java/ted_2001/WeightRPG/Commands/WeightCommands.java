...
public class WeightCommands implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (sender instanceof ConsoleCommandSender || sender instanceof RemoteConsoleCommandSender) {
            handleConsoleCommand(sender.getServer(), args);
            return true;
        }
        return true;
    }
    ...
    private void handlePlayerCommand(Player player, String[] args) {
        switch (args.length) {
            case 0 -> showWeight(player);
            case 1 -> handleSingleArgument(player, args[0]);
            case 2 -> handleTwoArguments(player, args[0], args[1]);
            case 3 -> handleThreeArguments(player, args[0], args[1], args[2]);
            default -> unknownCommandMessage(player);
        }
    }
    ...
    private void handleSingleArgument(Player player, String subCommand) {
        if (subCommand.equalsIgnoreCase("reload")) {
            ...
            send(player, Messages.getMessages().getString(success ? "success-reload-message" : "fail-reload-message",
                    success ? getPlugin().getPluginPrefix() + "&aConfig and weight files reloaded successfully."
    ...
    private void handleThreeArguments(Player player, String subCommand, String argument, String rawValue) {
        if (subCommand.equalsIgnoreCase("set")) {
            if (player.hasPermission("weight.set")) setItemWeight(player, argument, rawValue); else noPermMessage(player);
        } else if (subCommand.equalsIgnoreCase("add")) {
            if (player.hasPermission("weight.add")) addItemWeight(player, argument, rawValue); else noPermMessage(player);
        } else if (subCommand.equalsIgnoreCase("custom") && argument.equalsIgnoreCase("add")) {
            if (player.hasPermission("weight.custom")) setHeldItemPersistentWeight(player, rawValue, false); else noPermMessage(player);
        } else if (subCommand.equalsIgnoreCase("boost") && argument.equalsIgnoreCase("add")) {
            if (player.hasPermission("weight.boost")) setHeldItemPersistentWeight(player, rawValue, true); else noPermMessage(player);
        } else {
            unknownCommandMessage(player);
        }
    }
    ...
    private void showItemWeight(Player player, String rawItemName) {
        ...
        if (material != null && globalItemsWeight.containsKey(material)) {
            String message = Messages.getMessages().getString("get-item-success-message",
                    getPlugin().getPluginPrefix() + "&aThe weight of &e" + itemName + " &ais &e"
                            + String.format("%.2f", globalItemsWeight.get(material)));
            send(player, message.replace("%item%", material.toString())
                    .replace("%itemweight%", String.format("%.2f", globalItemsWeight.get(material))));
            return;
        }
        String message = Messages.getMessages().getString("get-item-fail-message",
                getPlugin().getPluginPrefix() + "&cCouldn't find &e" + itemName + " &cin the weight files.");
        send(player, message.replace("%item%", itemName));
    }
    ...
    private void setItemWeight(Player player, String rawItemName, String rawValue) {
        ...
        for (File file : new File[]{new File(weightsDir, "Blocks Weight.json"),
                new File(weightsDir, "Tools And Weapons Weight.json"), new File(weightsDir, "Misc Items Weight.json")}) {
            try {
                JSONObject root = readJson(file);
                if (!replaceWeight(root, itemName, rawValue)) continue;
                writeJson(file, root);
                reloadWeightMaps();
                String message = Messages.getMessages().getString("set-item-success-message",
                        getPlugin().getPluginPrefix() + "&aYou successfully set the weight of &e" + itemName
                                + " &ato &e" + rawValue + " &a.");
                send(player, message.replace("%item%", itemName).replace("%itemweight%", rawValue));
                return;
        ...
        String message = Messages.getMessages().getString("set-item-fail-message",
                getPlugin().getPluginPrefix() + "&cCouldn't find the &e" + itemName + " &cin any of the weight files.");
        send(player, message.replace("%item%", itemName));
    }
    ...
    private void addItemWeight(Player player, String rawItemName, String rawValue) {
        ...
        if (existingWeight != null) {
            String message = Messages.getMessages().getString("add-item-found-message",
                    getPlugin().getPluginPrefix() + "&cThis item already exists in the weight files and it's weight value is &e"
                            + existingWeight + "&c.");
            send(player, message.replace("%itemweight%", String.valueOf(existingWeight)));
            return;
        }
        ...
        try {
            ...
            writeJson(file, root);
            reloadWeightMaps();
            String message = Messages.getMessages().getString("add-item-success-message",
    ...
    private void setHeldItemPersistentWeight(Player player, String rawValue, boolean boost) {
        ...
        if (item.getType().isAir()) {
            send(player, Messages.getMessages().getString(boost ? "boost-add-item-fail-message" : "custom-add-item-fail-message",
        ...
        if (boost) ItemLoreUtils.updateBoostItemLore(item, value); else ItemLoreUtils.updateItemLore(item, value);
        weightCalculator.calculateWeight(player);
        String message = Messages.getMessages().getString(boost ? "boost-add-item-success-message" : "custom-add-item-success-message",
                boost ? getPlugin().getPluginPrefix() + "&aSet boost weight &e" + rawValue + " &afor item."
                        : getPlugin().getPluginPrefix() + "&aSet custom weight &e" + rawValue + " &afor item.");
        send(player, message.replace(boost ? "%boostweight%" : "%itemweight%", rawValue));
    }

    private void handleConsoleCommand(Server server, String[] args) {
        if (args.length == 0) {
            ...
            boolean success = reloadCommand();
            server.getConsoleSender().sendMessage(getPlugin().getPluginPrefix() + ChatColor.GRAY
                    + (success ? "Config and weight files reloaded successfully." : "There was an error while reloading."));
        }
    }
    ...
    private boolean reloadCommand() {
        ...
        if (config.exists()) getPlugin().reloadConfig(); else getPlugin().saveDefaultConfig();
        if (messages.exists()) Messages.reloadMessagesConfig(); else Messages.create();
        CalculateWeight.refreshThresholdValues();
        if (getPlugin().task != null && !getPlugin().task.isCancelled()) getPlugin().task.cancel();
        getPlugin().scheduler();
        if (!blocksWeight.exists() || !toolsWeight.exists() || !miscWeight.exists()) jsonFile.saveJsonFile();
        boolean success = reloadWeightMaps();
        for (Player onlinePlayer : getPlugin().getServer().getOnlinePlayers()) weightCalculator.calculateWeight(onlinePlayer);
        getPlugin().reloadPluginPrefix();
        return success;
    }

    private boolean reloadWeightMaps() {
        globalItemsWeight.clear();
        customItemsWeight.clear();
        boostItemsWeight.clear();
        jsonFile.successfullyRead = true;
        try {
            jsonFile.readJsonFile();
            return jsonFile.successfullyRead;
        } catch (RuntimeException exception) {
            jsonFile.successfullyRead = false;
            getPlugin().getLogger().severe("Unable to read weight files: " + exception.getMessage());
            return false;
        }
    }
    ...
    private void writeJson(File file, JSONObject root) throws IOException {
        try (PrintWriter writer = new PrintWriter(new FileWriter(file))) {
            ...
            if (writer.checkError()) throw new IOException("Failed while writing " + file.getName());
        }
    }
    ...
    private boolean replaceWeight(JSONObject root, String itemName, String rawValue) {
        ...
        while (keys.hasNext()) {
            ...
            if (!(section instanceof JSONArray values)) continue;
    ...
    private void commandHelp(Player player, String permission, String path, String fallback) {
        if (!player.hasPermission(permission)) noPermMessage(player); else send(player, Messages.getMessages().getString(path, fallback));
    }
