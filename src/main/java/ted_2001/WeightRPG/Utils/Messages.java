...
public class Messages {
    ...
    public static void getDefaults(){
        try {
            ...
            configuration.addDefault("receive-item-message", "%displayname% &aYou receive &c%amount% &aof &e%block% &awhich weights &8%totalweight%&a.");
            configuration.addDefault("receive-item-message-cooldown", 1);
            configuration.addDefault("receive-item-message-enabled", true);
            configuration.addDefault("lost-item-message", "%displayname% &aYou lost &e%block% &awhich weights &8%totalweight%&a.");
            configuration.addDefault("lost-item-message-cooldown", 1);
            configuration.addDefault("lost-item-message-enabled", true);
            configuration.addDefault("place-block-message", "%displayname% &aYou place &e%block% &awhich weights &8%itemweight%&a.");
            ...
            comments.add("You can use all placeholders %block% for item name, %itemweight% for items weight , %amount% for items amount and %totalweight% for total items weight.");
            configuration.setComments("receive-item-message", comments);
            configuration.setComments("disable-jump-message", Collections.singletonList("The message that will send to the player when he is carrying too many items to jump."));
